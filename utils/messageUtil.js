...
export function testAlyPushMsg(bean, req, res, cb) {
  let {input, output} = bean;
  let push = new ALY.PUSH({
        accessKeyId: 'LTAItaHLZ7Y32qTR',//'<your access key id>',
        secretAccessKey: 'sYmldHmwfXmNRPviuffN2AnmSLawK9',//'<your access key secret>',
  ...
  push.push({
    AppKey: '24825622',//   iOS:'24827695',//'<your appKey>',
    ...
    DeviceType: "ALL",
    Title: 'Rico Test Push Title',
    Body: 'Rico Test Push Body',
    AndroidXiaoMiNotifyTitle: 'RicoTestTitle', //Max16
    AndroidXiaoMiNotifyBody: '1Rico Test Push Body',
    //iOS相关配置
    ...
    if(err) {
      console.log('[alyPush] err:', err);
      return cb && cb(err);
    }

    console.log('[alyPush] res:', res);
    input.resResult = res;
    return cb && cb(null);
  });
}
